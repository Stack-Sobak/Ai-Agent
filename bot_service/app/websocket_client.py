...
class BotWebSocketClient:
    ...
    def __init__(self, bot, backend_url: str, bot_id: str):
        self.bot = bot
    ...
    async def listen_global(self):
        uri = f"{self.backend_url}/ws/global?botId={self.bot_id.split( )[1]}"
    ...
    async def listen_private(self):
        uri = f"{self.backend_url}/ws/private?botId={self.bot_id.split( )[1]}"
